...
/**
 * There's no real sign-in behind this account — it exists purely so the
 * Contacts Provider will accept RawContacts under our account type, which is
 * what unlocks adding a "connected apps" row to an existing device contact
 * (aggregated in by matching phone number). See AccountUtils.kt.
 */
class SamChatAuthenticator(context: Context) : AbstractAccountAuthenticator(context) {
